...
# Pricing per day
PRICE_PER_DAY = 50  # Set your price per day
...
def get_rentals():
    ...
    rentals = [
        {'id': r[0], 'name': r[1], 'car_model': r[2], 'rental_date': str(r[3]), 'return_date': str(r[4]), 'total_amount': r[5]} 
        for r in results
    ]
    cursor.close()
...
def update_rental(rental_id):
    data = request.json
    if 'rental_date' not in data or 'return_date' not in data:
        return jsonify({'error': 'Invalid data'}), 400
